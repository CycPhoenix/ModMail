...
module.exports = ({ bot, knex, config, commands }) => {
    ...
    commands.addInboxThreadCommand('role', [], async (msg, args, thread) => {
        const displayRole = await getModeratorThreadDisplayRoleName(msg.member, threa.id)
        if (displayRole) {
    ...
    commands.addInboxThreadCommand('role', '<role:string$>', async (msg, args, thread) => {
        ...
        if (!role || !msg.member.rooles.includes(role.id)) {
    ...
    commands.addInboxServerCommand('role', [], async (msg, args, thread) => {
        ...
        if (displayRole) {
            msg.channel.createMessage(`Your defautl display role is currently **${displayRole}**`)
        } else {
