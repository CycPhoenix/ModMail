        ...
        bot.once('ready', async () => {
            ...
            await (new Promise(resolve => {
                const waitNoteTimeout = setTimeout(() => {
                    ...
                    if (isSingleServer) {
                        console.log('WARNNING: The bot will not work before it\'s invited to the server.')
                    } else {
...
function initBaseMessageHandlers() {
    ...
    bot.on('messageCreate', async msg => {
        ...
        const thread = await callBeforeNewThreadHooks.findByChannelId(msg.channel.id)
        if (!thread) return
